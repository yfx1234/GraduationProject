...
struct GRADUATIONPROJECT_API FDroneParameters
{
    ...
    /** @brief 绕 X 轴的转动惯量 kg·m²（由 ComputeInertiaMatrix 自动计算） */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Drone|Parameters|Inertia")
    double Jx = 0.0023;

    /** @brief 绕 Y 轴的转动惯量 kg·m²（由 ComputeInertiaMatrix 自动计算） */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Drone|Parameters|Inertia")
    double Jy = 0.0023;

    /** @brief 绕 Z 轴的转动惯量 kg·m²（由 ComputeInertiaMatrix 自动计算） */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Drone|Parameters|Inertia")
    double Jz = 0.004;
    ...
    /** @brief 计算得到的推力系数 kT = C_T * ρ * D⁴（N/(rev/s)²） */
    double ThrustCoefficient = 1.0e-5;

    /** @brief 计算得到的力矩系数 kQ = C_P * ρ * D⁵ / (2π)（N·m/(rev/s)²） */
     ...
     * 基于 AirSim 公式：
     *   MaxThrust = C_T * ρ * n² * D⁴
     *   MaxTorque = C_P * ρ * n² * D⁵ / (2π)
     *   kT = MaxThrust / MaxSpeed²（转换为 rad/s 制式）
     *   kQ = MaxTorque / MaxSpeed²
     */
     ...
     *   Jx = m_box/12 * (By² + Bz²) + Σ(yi² + zi²) * m_motor
     *   Jy = m_box/12 * (Bx² + Bz²) + Σ(xi² + zi²) * m_motor
     *   Jz = m_box/12 * (Bx² + By²) + Σ(xi² + yi²) * m_motor
     */
