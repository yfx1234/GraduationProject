"""Turret actor wrapper."""

from .AgentBase import AgentBase


class AgentTurret(AgentBase):
    """Object wrapper for common turret commands."""

    def __init__(self, client, actor_id="turret_0", classname="TurretPawn", label="Turret"):
        super().__init__(client=client, actor_id=actor_id, classname=classname, label=label, spawn_unit="m")

    def call_command(self, function_name, **payload):
        request_payload = {"function": function_name, "id": self.actor_id}
        request_payload.update(payload)
        self.last_response = self.client.request({"call_turret": request_payload})
        return self.last_response

    def set_angles(self, pitch, yaw):
        return self.call_command("set_angles", pitch=float(pitch), yaw=float(yaw))

    def fire(self, speed=400.0):
        return self.call_command("fire", speed=float(speed))

    def start_tracking(self, target_id):
        return self.call_command("start_tracking", target_id=str(target_id))

    def stop_tracking(self):
        return self.call_command("stop_tracking")

    def show_prediction_line(self):
        return self.call_command("show_prediction")

    show_prediction = show_prediction_line

    def hide_prediction_line(self):
        return self.call_command("hide_prediction")

    hide_prediction = hide_prediction_line

    def reset(self):
        return self.call_command("reset")

    def get_state(self):
        self.last_response = self.client.request({"get_turret_state": {"id": self.actor_id}})
        return self.last_response

    state = get_state


TurretAgent = AgentTurret