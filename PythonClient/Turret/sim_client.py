...
class SimClient:
    ...
    def drone_set_camera_angles(self, pitch: float, yaw: float, drone_id: str = "drone_0") -> dict:
        return self._send({"call_drone": {"function": "set_camera_angles",
                           "id": drone_id, "pitch": pitch, "yaw": yaw}})
    ...
    # ---- Image ----
    def get_image_base64(self, agent_id: str = None) -> Optional[str]:
        cmd = {"get_image": {"id": agent_id} if agent_id else {}}
        resp = self._send(cmd)
        if resp.get("status") == "ok":
            return resp.get("data")
        return None

    def get_image_raw(self, agent_id: str = None) -> Optional[dict]:
        """返回完整的图像响应（含 camera_pos, camera_rot, fov 等元数据）"""
        cmd = {"get_image": {"id": agent_id} if agent_id else {}}
        resp = self._send(cmd)
        if resp.get("status") == "ok":
            return resp
    ...
    def get_image_bytes(self, agent_id: str = None) -> Optional[bytes]:
        b64 = self.get_image_base64(agent_id)
        if b64:
            return base64.b64decode(b64)
        return None

    def get_image_numpy(self, agent_id: str = None) -> Optional[np.ndarray]:
        """返回 numpy 格式的 BGR 图像（需要 opencv）"""
        import cv2
        img_bytes = self.get_image_bytes(agent_id)
        if img_bytes:
